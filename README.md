...
- 支持自定义触发键开始录音 (默认 `Right Shift`)
- 松开后自动结束识别并注入当前输入框
- 托盘常驻，双击可打开控制台
- 优先保证实时性和稳定性 (不抢焦点 `WS_EX_NOACTIVATE`)

## 当前能力

- 托盘常驻与托盘右键菜单
- 自定义单键按住说话 (RShift / CapsLock / F-Keys 等)
- HUD 底部状态提示、音量波形与平滑文字动效
- 基于名称匹配的麦克风助手 (插拔设备不丢配置)
- 火山引擎大模型流式 ASR (带业务级错误提示)
- 剪贴板注入与恢复
- 控制台窗口
- 开机自启开关
- ASR / LLM 设置窗口
